...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`which`](https://crates.io/crates/which).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `which` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Cross-platform `which` (locate executables on PATH)
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:which-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `which` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
