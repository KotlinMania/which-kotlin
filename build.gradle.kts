...
kotlin {
    ...
    // Maximal Kotlin 2.3.21 non-JVM target coverage. CodeQL gets a separate
    // JavaExec extraction task below instead of a published jvm() target.
    val xcf = XCFramework("Which")
    ...
    swiftExport {
        moduleName = "Which"
        flattenPackage = "io.github.kotlinmania.which"
    }

    android {
        namespace = "io.github.kotlinmania.which"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "which-kotlin", version.toString())

    pom {
        name.set("which-kotlin")
        description.set("Kotlin Multiplatform port of harryfei/which-rs - A Rust equivalent of Unix command \"which\". Locate installed executable in cross platforms")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/which-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/which-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/which-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/which-kotlin.git")
        }
    }
}

// CodeQL Java/Kotlin extraction task. This compiles commonMain plus the
// Android actuals with the embeddable JVM compiler only for CodeQL and does
// not publish a JVM target.
...
val patchWasmWasiNodePreopens = tasks.register("patchWasmWasiNodePreopens") {
    ...
    doLast {
        val runnerFile = layout.buildDirectory.file(
            "compileSync/wasmWasi/test/testDevelopmentExecutable/kotlin/which-kotlin-test.mjs",
        ).get().asFile
