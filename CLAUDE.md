# Claude Code Project Instructions — which-kotlin

## Project Overview

This is **which-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`which`](https://crates.io/crates/which).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/which/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/which>
